...
public function my_niga($me,$numero)
    {		
        $data = array(
           'user_number'        => $me,
           'friend_number'      => $numero,
		   'friend_name'        => $numero,
	       'timestamp'          => time()
        );
      ...
      $this->db->insert('begoo_friends',$data);	
	   ...
	   //je regarde son nom pour le mettre dans la liste des amis
	  $name = $this->is_user($numero);//on regarde s'il est membre
	  
	    if($name)
		{
		  //on met à jour son nom
		  $this->db->where('friend_number', $numero);
          $this->db->update('begoo_friends', array('friend_name' => $name));
        }		  
	} 
